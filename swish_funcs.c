...
int run_command(strvec_t *tokens) {
    ...
    char *args[MAX_ARGS + 1]; //build blank string array for arguments needed later in exec

    for (int i = 0; i < tokens->length; i++) { //copy over arguments into string array
        args[i] = strvec_get(tokens, i);
    }
    //note: we are using ->'s instead of .'s because tokens is a pointer

    args[tokens->length] = NULL; //ensure null sentinal is at the end of arg array

    execvp(args[0], args); //run the desired command (reimages the process)

    perror("exec"); //if this runs then the execvp did not reimage our process and that's an error state
    return -1;
...
int resume_job(strvec_t *tokens, job_list_t *jobs, int is_foreground) {
    ...
    if (tokens->length < 2) { //check for enough tokens
        printf("Not enough tokens in resume_job");
        return -1;
    }
    int index = atoi(strvec_get(tokens, 1)); //grab the job index from str vector

    job_t *job = job_list_get(tokens, index); //get actual job information
    if (job == NULL) {
        printf("Job doesn't exist in resume_job");
        return -1;
    }
